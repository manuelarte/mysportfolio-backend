...
public class UserTeamCommandController {
  ...
  public ResponseEntity<UserTeamDto> saveUserTeam(
    ...
    if (userTeamDto.getUserInTeam() != null) {
      ...
      userInTeamSaved = teamToUsersQueryService.findByTeamId(teamSaved.getId())
          .orElseThrow(() -> new EntityNotFoundException("UserInTeam not found for team " + teamSaved.getId())).getUsers()
          .get(getUserId());
    }
  ...
  public ResponseEntity<UserTeamDto> updateUserTeam(
    ...
    if (userTeamDto.getUserInTeam() != null) {
      ...
      userInTeamSaved = teamToUsersQueryService.findByTeamId(teamUpdated.getId())
          .orElseThrow(
              () -> new EntityNotFoundException(String.format("Can't find team %s in TeamToUsers", teamUpdated.getId())))
          .getUsers().get(getUserId());
    }
