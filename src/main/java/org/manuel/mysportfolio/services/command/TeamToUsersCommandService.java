...
public interface TeamToUsersCommandService {
  ...
  @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_SYSTEM' ) or (hasRole('ROLE_USER') "
      + "and @permissionsService.isTeamAdmin(#teamToUsers.teamId))")
  TeamToUsers update(TeamToUsers teamToUsers);

  @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_SYSTEM' ) or (hasRole('ROLE_USER') "
      + "and (@permissionsService.isTeamAdmin(#teamId) or @permissionsService.canSaveUserInTeam(#teamId, #userId)))")
