...
public class QueryCriteriaToMongoQueryTransformer implements
    BiFunction<QueryCriteria, Class<?>, Query> {
  ...
  public Query apply(final QueryCriteria queryCriteria, final Class<?> clazz) {
    ...
    final Map<QueryCriteria.QueryOption, List<Criteria>> mapped = groupByQueryOption.asMap()
        .keySet().stream().collect(Collectors.toMap(Function.identity(),
            x -> groupByQueryOption.get(x).stream().map(s -> createCriteria(s, clazz))
  ...
  private Multimap<QueryCriteria.QueryOption, SearchCriterion> groupByQueryOption(
      final QueryCriteria queryCriteria) {
    final Multimap<QueryCriteria.QueryOption, SearchCriterion> groupByQueryOption = ArrayListMultimap
        .create();
  ...
  private <T> Criteria createCriteria(final SearchCriterion<String, Object> searchCriterion,
      final Class<T> clazz) {
    ...
    try {
      ...
      if (searchCriterion.getValue() instanceof Iterable) {
        converted = typeConversionService
            .convertToList((Iterable) searchCriterion.getValue(), field.getType());
      } else {
      ...
      throw new RuntimeException(String
          .format("Field %s not found in class %s", searchCriterion.getKey(),
              clazz.getSimpleName()));
    }
    return criteria;
  }
